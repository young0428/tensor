...
def build_net(name):
	...
	with tf.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
		...
		X1 = tf.reshape(X_price,[batch_size,int(day),int(13)])
		lstm1 = tf.contrib.cudnn_rnn.CudnnLSTM(3,n_hidden,dropout=dropout)
		outputs1,states = lstm1(X1)
		outputs1 = tf.transpose(outputs1,[1,0,2])
		outputs1 = outputs1[-1]
		L1 = tf.layers.dense(outputs1,128,activation=None)

		X2 = tf.reshape(X_volume,[batch_size,int(day/2),int(13*2)])
		lstm2 = tf.contrib.cudnn_rnn.CudnnLSTM(3,n_hidden,dropout=dropout)
		outputs2,states = lstm2(X2)
		outputs2 = tf.transpose(outputs2,[1,0,2])
		outputs2 = outputs2[-1]

		L2 = tf.layers.dense(outputs2,128,activation=None)


		L = tf.concat([L1,L2],1)
		
		L = tf.layers.dense(L,64,activation=tf.nn.relu)
