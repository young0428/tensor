...
def generator(inputs):
	...
	with tf.compat.v1.variable_scope("generator", reuse=tf.compat.v1.AUTO_REUSE):
		...
		outputs = outputs[-1]
		output = tf.layers.dense(outputs,512,activation=tf.nn.leaky_relu)
...
train_G = tf.train.AdamOptimizer(0.00001).minimize(loss_G,var_list=vars_G)
...
for episode_step in range(episode_step+1,episode_step+100001):
	...
	if(episode_step%200 == 0):
		...
		plt.savefig('./result3/{}.png'.format(str(episode_step).zfill(4)))
