...
class DQN:
	...
	def get_action(self):
		Q_value = self.sess.run(self.Q, feed_dict={self.input_X:self.index_to_dic(self.cur_index),self.keep_prob:1})
	...
	def train(self,reward,action):
		target_Q_value = self.sess.run(self.target_Q,feed_dict={self.input_X:self.index_to_dic(self.next_index),self.keep_prob:1})
		Y = []
